...
votaciones  		<- read.csv('source.csv', header=TRUE, strip.white=TRUE, encoding='UTF-8')
...
result				<- wnominate(rc, polarity = c(1,49), verbose = T, minvotes = 1000)
summary(result)
par(mfrow = c(1,1))
plot.coords(result)
write.table(result$legislators, file='out_r.csv', row.names_= TRUE, sep = ',')
